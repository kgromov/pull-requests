...
router.get("/", async (req, res) => { 
    ...
    const nextSprintNumber = sprints.reduce((s1, s2) => s1.number > s2.number ? s1.number : s2.number);
...
router.get("/:sprintNumber", async (req, res) => {
    ...
    const prevSprint = await sprintService.getSprintSummary(sprintNumber - 1);
    const currSprint = await sprintService.getSprintSummary(sprintNumber);
