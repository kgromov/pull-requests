...
function initData() {
    ...
    devsContainer.append('<p>jQuery finally included!</p>');
    return;
...
async function loadDevelopers(sprintNumber) {
    const developers = []; 
    await fetch(dirName + `/resources/data/pr-summary-sprint-${sprintNumber}.json`)
        .then(res => res.json())
...
async function initDOM(prevSprint, currentSprint) {
    const prevSummary = await loadDevelopers(prevSprint); 
    const currentSummary = await loadDevelopers(currentSprint);
